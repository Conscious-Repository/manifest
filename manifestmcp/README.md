...
| Tool | Contract |
| --- | --- |
| `email.prepare` | Prepare an immutable email for owner review in Chat and Feed. Specify the correspondence domain (aion or ooda) for external recipients. Personal is unavailable. Never sends; execution requires owner approval. Attachments are exact content-addressed files owned by that domain. Set monitorReplies when the owner requests ongoing reply tracking after sending. |
...
| `sources.list` | Read the application's shared adapter registry and scope fields; no fetch or cache sweep. `max` is the optional people/display cap per run (default 25, ceiling 100), not an upstream page size. PubMed reads a separate paper budget, `fields.papers` (default 100, ceiling 500), and aggregates every author on those papers into people. |
