...
func TestRetiredFinishLineFieldsStripOnSave(t *testing.T) {
	// until/verify/kpi retired 2026-08-19 (owner call): recognized so they
	// never pass through as unknown fields, rebuilt from nothing — a
	// round-trip DROPS them from live lines while frozen history keeps its
	// bytes. The migration detector fires exactly on that difference.
	in := "# Goals\n\n## Aion\n\n### 1-year — 2026\n" +
	...
	// and quiet on a file whose only tokens are frozen
	if !hasRetiredFields(in) {
		t.Fatal("detector should fire while live lines carry retired tokens")
	}
	if hasRetiredFields(out) {
		t.Fatalf("detector should be quiet after the strip:\n%s", out)
	}
	// stripped output is a fixpoint
