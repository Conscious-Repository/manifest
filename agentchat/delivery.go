...
func creationSignature(profile, title, model string, origin *Origin) string {
	...
	if origin != nil {
		// Side-chat context is generated by the server after recovery; mutable
		// source history must not change the accepted creation signature.
		copy := *origin
		if copy.Mode == "side" {
			copy.Context = ""
			copy.HistoryOmitted = 0
		}
		text += "\x00" + originJSON(&copy)
	}
	return fingerprint(text)
}
