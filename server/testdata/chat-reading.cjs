...
// Planning replies must repaint a native-root chat even when its native file
// offset has not advanced. Stop-triggered final reads use the same path.
(async()=>{
 ...
 const tail=vm.createContext({chatQuestionPanel:()=>{},chatTermFind:()=>root.se,renderChatInboxRows:()=>{},document:{querySelector:()=>null},chatTermOpen:root,chatTermTailing:false,chatTermBase:id=>'/native/'+id,chatTermReadHealth(){},AbortController,setTimeout,clearTimeout,fetch:async()=>({ok:true,json:async()=>response}),chatTermPaintTurns:()=>paints++,chatTermRepaintHead:()=>{}});
