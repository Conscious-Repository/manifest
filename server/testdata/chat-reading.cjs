...
(async()=>{
 ...
 const tail=vm.createContext({chatQuestionPanel:()=>{},chatTermFind:()=>root.se,renderChatInboxRows:()=>{},document:{querySelector:()=>null},chatTermOpen:root,chatTermTailing:false,chatTermBase:id=>'/native/'+id,AbortController,setTimeout,clearTimeout,fetch:async()=>({ok:true,json:async()=>response}),chatTermPaintTurns:()=>paints++,chatTermRepaintHead:()=>{}});
