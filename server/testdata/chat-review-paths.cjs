...
await page.getByText('2 changed files',{exact:true}).waitFor();assert.equal(await page.locator('[data-before-line="4"] code').textContent(),'-old');assert.equal(await page.locator('[data-after-line="7"] code').textContent(),'+new');await page.getByRole('combobox',{name:'Changed file'}).selectOption('1');await page.getByText('+<script>bad()</script>',{exact:true}).waitFor();
