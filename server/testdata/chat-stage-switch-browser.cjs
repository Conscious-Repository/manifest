...
(async()=>{
 ...
 try{
  ...
  const extraFiles=[];
  await page.route('**/api/artifacts?**',async route=>{fileReads++;const fail=fileFail;await new Promise(r=>setTimeout(r,fileDelay));return route.fulfill(fail?{status:503,json:{error:'offline'}}:{json:{artifacts:retainedOutputs.length?[...extraFiles,{id:'cccccccccccccccc',title:'Saved native output',head:'c'.repeat(64),ref:'output.md',kind:'report'}]:[]}});});
  ...
  extraFiles.push(...Array.from({length:60},(_,i)=>({id:'file-'+i,title:'Saved file '+i,head:'d'.repeat(64),ref:'file-'+i+'.md',kind:'report'})));
  await page.evaluate(()=>window.dispatchEvent(new Event('manifest-artifacts-changed')));
  await page.locator('.chat-files-list').getByRole('button',{name:'Saved file 59',exact:true}).waitFor();
  await page.locator('.chat-files-list').evaluate(e=>{e.scrollTop=650;});
  const visibleFile=()=>page.locator('.chat-files-list').evaluate(e=>{const top=e.getBoundingClientRect().top;const row=[...e.children].find(r=>r.getBoundingClientRect().bottom>top);return {text:row?.querySelector('button')?.textContent,offset:row?.getBoundingClientRect().top-top,scroll:e.scrollTop};});
  const beforeRefresh=await visibleFile();assert.ok(beforeRefresh.scroll>0);
  fileDelay=500;extraFiles.unshift({id:'new-first',title:'Saved newest file',head:'e'.repeat(64),ref:'new.md',kind:'report'});
  await page.evaluate(()=>window.dispatchEvent(new Event('manifest-artifacts-changed')));
  await page.waitForTimeout(100);
  assert.deepEqual(await visibleFile(),beforeRefresh,'refresh keeps usable files and reading position while waiting');
  await page.locator('.chat-files-list').getByRole('button',{name:'Saved newest file',exact:true}).waitFor();
  const afterRefresh=await visibleFile();assert.equal(afterRefresh.text,beforeRefresh.text,'insertion above keeps the visible file');assert.ok(Math.abs(afterRefresh.offset-beforeRefresh.offset)<2,'visible file offset is retained');
  fileFail=true;
  await page.evaluate(()=>window.dispatchEvent(new Event('manifest-artifacts-changed')));
  await page.getByText('Files could not be loaded. Refresh to retry.',{exact:true}).waitFor();
  assert.deepEqual(await visibleFile(),afterRefresh,'failed refresh retains usable files and position');
  await page.getByLabel('File scope',{exact:true}).selectOption('all');
  await page.getByText('Files unavailable.',{exact:true}).waitFor();
  assert.equal(await page.locator('.chat-files-list .chat-file-row').count(),0,'a failed different scope does not display old results');
  fileFail=false;fileDelay=0;
  await page.getByLabel('File scope',{exact:true}).selectOption('conversation');
  await page.locator('.chat-files-list').getByRole('button',{name:'Saved newest file',exact:true}).waitFor();
