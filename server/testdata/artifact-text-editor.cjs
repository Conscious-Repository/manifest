...
await page.evaluate(()=>{
 preview.close();window.ChatDraftState=class{value=null;async refresh(){}set(value){this.value=value;}async flush(){}};
 window.chatStateEqual=(a,b)=>JSON.stringify(a)===JSON.stringify(b);
 window.editOptions={load:async()=>structuredClone(a),save:async()=>{throw Error('Restoration must never save');}};
 preview=artifactWorkspace(document.querySelector('main'),editOptions);
});
await page.getByRole('button',{name:'Edit',exact:true}).click();
await page.getByLabel('File content').fill('draft line\n'.repeat(80));
await page.getByLabel('File content').evaluate(e=>{e.setSelectionRange(12,24);e.scrollTop=90;});
await page.evaluate(async()=>{window.savedEdit=preview.getView();preview.close();preview=artifactWorkspace(document.querySelector('main'),editOptions);await preview.restoreView(savedEdit);});
assert.equal(await page.getByLabel('File content').inputValue(),'draft line\n'.repeat(80));
assert.deepEqual(await page.getByLabel('File content').evaluate(e=>[e.selectionStart,e.selectionEnd]),[12,24]);
assert.equal(await page.getByLabel('File content').evaluate(e=>e.scrollTop),90);
assert.equal(await page.evaluate(()=>preview.getView().mode),'edit');
await page.getByRole('button',{name:'Review changes',exact:true}).click();
await page.getByRole('button',{name:'Edit text',exact:true}).waitFor();
await page.evaluate(async()=>{const view=preview.getView();preview.close();preview=artifactWorkspace(document.querySelector('main'),editOptions);await preview.restoreView(view);});
await page.getByRole('button',{name:'Edit text',exact:true}).waitFor();
assert.equal(await page.evaluate(()=>preview.getView().mode),'edit-review');
