...
const ctx=vm.createContext({AbortController,setTimeout,clearTimeout,chatTermMerge:(a,b)=>a.push(...b),chatTermReadHealth(){},chatTerminalLoadTicket:0,chatRouteVersion:1,chatAgent:'codex',chatOpenId:'one',chatTermOpen:null,
 ...
 chatTermOpenFrom:(id,se,d)=>({id,se,turns:d.turns||[],offset:d.offset||0,signature:d.signature,live:false}),chatTermSignature:o=>o.signature,
...
(async()=>{
 ...
 ctx.chatTermOpen.turns=[{who:'assistant',text:'Keep native reply'}];ctx.chatTermOpen.offset=42;
 const missing=ctx.loadChatTermSession('one');const retained=ctx.chatTermOpen;requests.at(-1).resolve({ok:true,json:async()=>({historyAvailable:false,offset:0,turns:[]})});await missing;assert.deepEqual(ctx.chatTermOpen.turns,retained.turns,'unavailable history preserves cached turns');assert.deepEqual(ctx.chatTermOpen.se.run,retained.se.run);assert.equal(ctx.chatTermOpen.offset,42);
