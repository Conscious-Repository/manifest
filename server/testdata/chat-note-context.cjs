...
(async()=>{const browser=await chromium.launch({headless:true});try{
 ...
 await p.evaluate(()=>{
  ...
  window.fetch=async url=>{const params=new URL(url,location.href).searchParams,kind=params.get('kind');const ids=kind==='note'?['one/same.md','two/same.md']:kind==='task'?['inbox/first','work/second']:kind==='person'?['alice','crm/alice']:kind==='candidate'?['cand/one','cand/two']:kind==='project'?['project/one','project/two']:['work/annual','work/stage'];const record=id=>({kind,id,title:kind==='note'?id:'Same title',detail:id,route:'#/'+(kind==='note'?'note':kind==='person'?'contacts':kind==='project'?'chat/project':kind==='candidate'?'aion/recruiting/candidate':kind+'s')+'/'+encodeURIComponent(id)});return {ok:true,json:async()=>url.includes('/preview?')?{record:record(params.get('id')),content:'Exact literal <script>source</script>\n'.repeat(35),revision:previewRevision}:{records:ids.map(record)}};};
 ...
 for(const kind of ['task','goal','person','project','candidate']){
  ...
  const id=kind==='task'?'work/second':kind==='person'?'crm/alice':kind==='project'?'project/two':kind==='candidate'?'cand/two':'work/stage';assert.deepEqual(await p.evaluate(()=>retainRequests.at(-1).payload),{kind,id,revision:'b'.repeat(64)});
  await p.evaluate(async()=>{const view=api.getView();api.close();chatOpenRecords();await api.restoreView(view);});assert.equal(await p.getByLabel('Record kind').inputValue(),kind);assert.equal(await p.getByRole('link',{name:'open source '+kind}).getAttribute('href'),'#/'+(kind==='person'?'contacts':kind==='project'?'chat/project':kind==='candidate'?'aion/recruiting/candidate':kind+'s')+'/'+encodeURIComponent(id));
 }
