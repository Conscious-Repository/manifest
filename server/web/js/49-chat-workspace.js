  ...
  const render=()=>{
   ...
   for(const {artifact:a,roles,attachment} of rows){
    ...
    for(const link of a.sources||[]){
     ...
     const anchor=el('a','sprt-quiet chat-file-source',link.kind==='conversation'?'source conversation':link.kind==='execution'?'producing execution':['note','task','goal','person','project','candidate','organization','schedule','calendar'].includes(link.kind)?'source '+link.kind:'producing run');anchor.href=link.route;anchor.title=link.label||link.id;item.append(anchor);
    }
