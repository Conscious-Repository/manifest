  ...
  const render=()=>{
   ...
   for(const {artifact:a,roles,attachment} of rows){
    ...
    for(const link of a.sources||[]){
     ...
     const anchor=el('a','sprt-quiet chat-file-source',link.kind==='conversation'?'source conversation':link.kind==='execution'?'producing execution':['note','task','goal','person','project','candidate','organization'].includes(link.kind)?'source '+link.kind:'producing run');anchor.href=link.route;anchor.title=link.label||link.id;item.append(anchor);
    }
...
function chatOpenRecords(){
 ...
 return chatEnsureWorkspace().tab('notes','Records',(host,drop)=>{
  ...
  return {element:pane,close:()=>{closed=true;++ticket;pane.remove();drop();},getView:()=>({query:ta.value(),kind:kind.value,id:selected?.record.id,revision:selected?.revision,scrollTop:preview.scrollTop}),restoreView:async view=>{kind.value=['note','task','goal','person','project','candidate','organization'].includes(view.kind)?view.kind:'note';ta.setValue(view.query||'');status.textContent=hints[kind.value];if(view.id||view.path)await load(view.id||view.path,view.revision);preview.scrollTop=Math.max(0,Number(view.scrollTop)||0);return true;}};
 },{kind:'records'});
}
