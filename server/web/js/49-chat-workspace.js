...
async function chatRestoreWorkspace(){
 ...
 for(const tab of saved.tabs){
  ...
  else if(tab.spec?.kind==='side'&&/^#\/chat\//.test(tab.spec.route||''))w.tab(tab.key,tab.spec.existing?tab.spec.title:'Side chat',host=>chatMountSideFrame(host,tab.spec),tab.spec);
 ...
 w.show(saved.open===true);w.restoring=false;if(typeof chatApplyWorkspaceLayout==='function')chatApplyWorkspaceLayout();
}
...
function chatEnsureWorkspace(restoring=false){
 ...
 const w={pane,body,entries,restoring,
  ...
  select(key){w.save();clearChooser();active=key;for(const [id,t] of entries){t.host.hidden=id!==key;t.button.setAttribute('aria-selected',String(id===key));t.button.tabIndex=id===key?0:-1;}w.show();if(typeof chatApplyWorkspaceLayout==='function')chatApplyWorkspaceLayout();},
  drop(key){const t=entries.get(key);if(!t)return;t.host.remove();t.row.remove();entries.delete(key);if(!disposed&&active===key){const next=Array.from(entries.keys()).at(-1);if(next)w.select(next);else w.chooser();}if(typeof chatApplyWorkspaceLayout==='function')chatApplyWorkspaceLayout();w.save();},
...
function chatWorkspaceChooser(host){
 ...
 if(typeof chatWorkspaceExistingChat==='function')action('Open conversation','Show an existing chat in another pane','chat',chatWorkspaceExistingChat);
...
function chatOpenFiles(){
 ...
 return chatEnsureWorkspace().tab('files','Files',(host,drop)=>{
  const pane=el('section','chat-files-inspector'),toolbar=el('div','chat-files-toolbar'),search=document.createElement('input'),scope=document.createElement('select'),refresh=el('button','sprt-quiet','refresh'),status=el('p','chat-workspace-hint'),list=el('div','chat-files-list');
  ...
  const render=()=>{
   ...
   for(const {artifact:a,roles,attachment} of rows){
    ...
    for(const link of a.sources||[]){
     ...
     const anchor=el('a','sprt-quiet chat-file-source',link.kind==='conversation'?'source conversation':link.kind==='execution'?'producing execution':['note','task','goal','person','project','candidate','organization','schedule'].includes(link.kind)?'source '+link.kind:'producing run');anchor.href=link.route;anchor.title=link.label||link.id;item.append(anchor);
    }
  ...
  const load=async()=>{
   ...
   try{
    ...
    else{query.set('conversation_backend',source.backend==='terminal'?'terminal':'agent');query.set('conversation_agent',source.agent);query.set('conversation_id',source.id);}
    ...
    const owner=source?(source.backend==='terminal'?'terminal:':'agent:')+source.agent+'/'+source.id:'';
    const [foundScope,task,uploads]=await Promise.all([get('/api/artifacts?'+query),!all&&source.task?get('/api/tasks/panel?id='+encodeURIComponent(source.task)):Promise.resolve(null),!all?get('/api/chat/files?owner='+encodeURIComponent(owner)):Promise.resolve({files:[]})]);
...
function chatOpenRecords(){
 ...
 return chatEnsureWorkspace().tab('notes','Records',(host,drop)=>{
  ...
  return {element:pane,close:()=>{closed=true;++ticket;pane.remove();drop();},getView:()=>({query:ta.value(),kind:kind.value,id:selected?.record.id,revision:selected?.revision,scrollTop:preview.scrollTop}),restoreView:async view=>{kind.value=['note','task','goal','person','project','candidate','organization','schedule'].includes(view.kind)?view.kind:'note';ta.setValue(view.query||'');status.textContent=hints[kind.value];if(view.id||view.path)await load(view.id||view.path,view.revision);preview.scrollTop=Math.max(0,Number(view.scrollTop)||0);return true;}};
 },{kind:'records'});
}
