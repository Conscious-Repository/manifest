...
function chatOpenWorkingArtifact(spec) {
  ...
  w.tab(tabKey,spec.plan?"Plan":"Review",(host,drop)=>artifactWorkspace(host,{
    ...
    canEdit:spec.plan ? null : a=>/\.(md|txt|json|csv|tsv|yaml|yml|toml|js|jsx|ts|tsx|py|go|html|css|sql|sh|xml|svg)$/i.test(a.ref||"") && !["task-plan","knowledge-context","task-context","goal-context","person-context","project-context","candidate-context","organization-context","schedule-context"].includes(a.provenance?.source),
