...
function artifactWorkspace(mount, options) {
  ...
  return {element:pane, close:()=>close.click(), isEditing:()=>editing, refresh,
    getView:()=>({revision:selected,versionNumber:selectedNumber,provenanceOpen:!!body.querySelector('.artifact-provenance')?.open,mode:previewMode,scrollTop:body.scrollTop,syntaxEnabled:body.querySelector('.artifact-code-preview,.working-changes-view')?.getHighlight?.(),linkSourceOpen:!!body.querySelector('.artifact-link-source')?.open,table:{scrollLeft:body.querySelector('.artifact-table-scroll')?.scrollLeft||0,sourceOpen:!!body.querySelector('.artifact-table-source')?.open},editor:editing&&editor?{scrollTop:editor.scrollTop,start:editor.selectionStart,end:editor.selectionEnd,direction:editor.selectionDirection}:null,expandedFiles:[...body.querySelectorAll('.working-file[open]')].map(f=>f.dataset.diffFile),collapsedHunks:[...body.querySelectorAll('.working-changes-view')].flatMap(v=>v.getCollapsedHunks?.()||[])}),
    restoreView:async view=>{await ready;if(!pane.isConnected||!body.clientHeight)return false;if(Number.isInteger(view.versionNumber)){const version=current.revisions.find(r=>r.n===view.versionNumber&&r.hash===view.revision);if(!version)return false;if(selected!==version.hash||selectedNumber!==version.n)await show(current,version.hash,version.n);if(selected!==version.hash||selectedNumber!==version.n)return false;}if((view.mode==="edit"||view.mode==="edit-review")&&opts.save&&editState?.value&&(!opts.canEdit||opts.canEdit(current))){
