...
// mode/statuses/sources/colourBy are THE LENS (social graph plan D-A, D-G):
// ego is the default — whole is bounded by the same ceiling, and strays are
// surfaced by the filters, not by drawing everything. `passed` starts hidden:
// looked at and declined is remembered, not shown.
const RG_DEFAULTS = {
  hops: 2, center: 0.4, repel: 2.5, link: 0.2, dist: 1.4,
  nodeSize: 0.8, linkWidth: 0.5, fade: 1.2, open: "forces", panel: false,
  mode: "ego", statuses: { in_touch: true, pursuing: true, bridge: true, stranger: true, passed: false },
...
function rgCanvas(data) {
  ...
  requestAnimationFrame(() => {
    ...
    const hueOf = {};
    sim.nodes.forEach((n) => { if (typeof n.hue === "number" && n.hue >= 0) hueOf[n.id] = n.hue; });
    sim.links.forEach((l) => {
      const e = l.edge;
      l.el = rgSVG("line", "rg-edge" + (e.inferred ? " inferred" : "") + (e.kind === "member_of" ? " rg-member" : ""), {});
      // the source's hue rides on the link to it (D-C): membership is visible
      // through the links whatever the fill is showing
      if (e.kind === "member_of" && hueOf[e.to] != null) l.el.style.setProperty("--src-hue", String(hueOf[e.to]));
    ...
    order.forEach((n) => {
      const g = rgSVG("g", "rg-node rg-" + n.kind + (typeof n.hue === "number" && n.hue >= 0 ? " has-hue" : ""), {});
      if (typeof n.hue === "number" && n.hue >= 0) g.style.setProperty("--src-hue", String(n.hue));
...
// rgColour flips the fill between the two channels without a rebuild: the
// classes move, the simulation keeps running.
function rgColour() {
  const st = rgInit();
  if (st.svgEl) st.svgEl.classList.toggle("by-source", rgOpts.colourBy === "source");
}
...
function rgControls(data) {
  ...
  card.append(rgSection("filters", "filters", (body) => {
    // MODE: ego is hops from you; whole is everything the lens lets through,
    // under the same ceiling. Both are bounded by construction.
    const modes = el("div", "rg-chips");
    [["ego", "around you"], ["whole", "everything"]].forEach(([k, label]) => {
      const b = el("button", "filter-chip" + (rgOpts.mode === k ? " on" : ""), label);
      b.title = k === "ego" ? "the people within a few hops of you" : "every person the filters let through, ranked and capped";
      b.onclick = () => { rgOpts.mode = k; rgSaveOpts(); rgLoad(); };
      modes.append(b);
    });
    body.append(modes);
    if (rgOpts.mode !== "whole") {
    ...
    RG_STATUSES.forEach(([k, label]) => {
      ...
      const box = el("input", "");
      box.type = "checkbox";
      box.checked = !!rgOpts.statuses[k];
      box.onchange = () => { rgOpts.statuses[k] = box.checked; rgSaveOpts(); rgLoad(); };
      row.append(box, el("span", "rg-swatch rg-" + k, ""), el("span", "", label));
      statuses.append(row);
    });
    const srcRow = el("label", "rg-check rg-check-source");
    const srcBox = el("input", "");
    srcBox.type = "checkbox";
    srcBox.checked = !!rgOpts.sources;
    srcBox.onchange = () => { rgOpts.sources = srcBox.checked; rgSaveOpts(); rgLoad(); };
    srcRow.append(srcBox, el("span", "rg-swatch rg-source", ""), el("span", "", "sources"));
    statuses.append(srcRow);
    body.append(statuses);
    // COLOUR BY: status is the question the graph exists to answer; source is
    // for the moments you want "who came from where" at a glance.
    const colour = el("div", "rg-chips");
    ...
    if (total) body.append(el("div", "rg-note", total + " more not drawn — narrow the filters, or stand on one of them"));
  }));
...
// rgProfile fetches the deterministic profile for one node once, then
// repaints. Never guessed: what is on file, wherever the person's row lives.
async function rgProfile(id) {
  const st = rgInit();
  st.profiles = st.profiles || {};
  if (st.profiles[id]) return st.profiles[id];
  st.profiles[id] = { loading: true };
  try {
    const r = await fetch("/api/aion/recruiting/graph/node?id=" + encodeURIComponent(id));
    st.profiles[id] = r.ok ? await r.json() : { error: await r.text() };
  } catch (e) {
...
// rgDecide is pursue / pass for a person a sweep named: the same accept and
// reject the queue uses, on the run and draft the node carries. Pursuing is
// what creates the record (D-D); passing writes the tombstone.
async function rgDecide(node, verb) {
  if (!node.run || !node.draft) return;
  const path = verb === "pursue" ? "accept" : "reject";
  const out = await recSourcesPost("/api/aion/recruiting/sources/" + path + "/" + node.run + "/" + node.draft, {},
    verb === "pursue" ? node.label + " is on the board" : "passed on " + node.label);
  if (out) { const st = rgInit(); st.profiles = {}; rgLoad(); }
}
...
function rgPanel(data) {
  ...
  if (!node) return null;
  const pending = rgProfile(node.id);
  const prof = (st.profiles || {})[node.id];
  ...
  // came from, the ties naming them — strongest first
  if (!prof || prof.loading) box.append(el("div", "rg-panel-hint", "reading the record…"));
  else if (prof.error) box.append(el("div", "rec-scaffold-err", prof.error));
  else {
    const org = [prof.title, prof.org].filter(Boolean).join(" · ");
    ...
    if ((prof.sources || []).length) {
      const srcs = el("div", "rg-chips");
      srcs.append(el("span", "micro-label", "from"));
      prof.sources.forEach((sr) => {
        const b = el("button", "filter-chip rg-src-chip", sr.label);
        b.title = "stand on " + sr.label;
        b.onclick = () => rgStand(sr.id);
        srcs.append(b);
      });
      box.append(srcs);
    }
    if ((prof.ties || []).length) {
      const ties = el("div", "rg-ties");
      ties.append(el("span", "micro-label", "ties · " + prof.ties.length));
      prof.ties.slice(0, 8).forEach((t) => {
        const line = el("div", "rg-tie");
        const who = el("button", "linkish", t.otherLabel);
        who.onclick = () => rgSelect(t.other);
        line.append(who, el("span", "micro-label", (t.kind || "").replace(/_/g, " ")));
        if ((t.works || []).length) line.append(el("span", "rg-tie-works", t.works.length + (t.works.length === 1 ? " work" : " works")));
        if (t.inferred) line.append(el("span", "micro-label rec-inferred", "inferred"));
        ties.append(line);
      });
      if (prof.ties.length > 8) ties.append(el("div", "rg-note", (prof.ties.length - 8) + " more"));
      box.append(ties);
    }
  }
  void pending;
  ...
  if (node.kind === "bridge" && node.run && node.draft) {
    ...
    pursue.title = "add them to the board — this is what creates a record";
    pursue.onclick = () => rgDecide(node, "pursue");
