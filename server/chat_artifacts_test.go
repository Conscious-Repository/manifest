...
func TestPortalCannotReadPrivateArtifactWorkspace(t *testing.T) {
	...
	for _, path := range []string{"/api/artifacts/get?id=0123456789abcdef&preview=1", "/api/artifacts?q=private&sources=1", "/api/artifacts/get?id=0123456789abcdef&content=1", "/api/artifacts/content?id=0123456789abcdef", "/api/tasks/plan/workspace?id=aion:context-test", "/api/agents/chat/kairos-private/sessions/20260910-120000-abcdef/share-review"} {
		rr := httptest.NewRecorder()
